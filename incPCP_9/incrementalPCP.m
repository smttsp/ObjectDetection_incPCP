...
for k=stats.kIni:stats.kEnd, 
  ...
  if showFlag
    ...
    S1=medfilt2(S1);
%     figure(1);    imshow(S1);
    [out]=coloring_cars(S1,reshape( showNormalize(curFrame), [Nrows, Ncols, nDims]));
%     figure(2);    imshow(out);
%     figure(3);  
%     figure(2), imshow(abs(S2))
    
    %S2 = rgb2gray(  reshape( S, [Nrows, Ncols, nDims]));
...
%     figure(5);
%     colored_frame = color_curframe(reshape( showNormalize(curFrame), [Nrows, Ncols, nDims]), S1 );
%     imshow(colored_frame); colormap gray;
%     figure(6);    imshow(reshape( showNormalize(L), [Nrows, Ncols, nDims])); colormap gray;
%     set(fig5, 'Position', [1000,300,900,700]);
    drawnow;
    if k == 250
        ''
    end
